    ...
    for (si=0; si<n; si++)
    {
        for (loop=0; loop<x->nb_loop; loop++)
        {
            for (i=0; i<x->nb_inPos; i++)
                ...
                x->inPos[i].mass->posX = x->inPos[i].influence * x->inPos[i].vec[si];
            for (i=0; i<x->nb_inForce; i++)
                // get inlet value and make it a force to the specified mass
                x->inForce[i].mass->forceX += x->inForce[i].influence * x->inForce[i].vec[si];
...
void pmpd_tilde_dsp(t_pmpd_tilde *x, t_signal **sp)
{
    ...
    for (i = 0; i < x->nb_inPos; i++)
        x->inPos[i].vec = x->inlet_vector[x->inPos[i].nbr_inlet % x->nb_inlet];
    for (i = 0; i < x->nb_inForce; i++)
        x->inForce[i].vec = x->inlet_vector[x->inForce[i].nbr_inlet % x->nb_inlet];
...
static inline int validate_inlet_io(t_pmpd_tilde *x, int idx)
{
    if (idx < 0 || (x->nb_inlet_lim && idx+1 > x->nb_inlet_lim)) {
        pd_error(x, "pmpd~: no inlet at index %i", idx);
        return 0;
    }
    return 1;
}

static inline int validate_outlet_io(t_pmpd_tilde *x, int idx)
{
    if (idx < 0 || (x->nb_outlet_lim && idx+1 > x->nb_outlet_lim)) {
        pd_error(x, "pmpd~: no outlet at index %i", idx);
        return 0;
    }
    return 1;
}

static void pmpd_tilde_need_outlet(t_pmpd_tilde *x, int idx)
{
...
// nbr_inlet, *mass, influence;
{
    ...
    x->inPos[x->nb_inPos].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inPos++;
}
...
void pmpd_tilde_inForce(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inForce[x->nb_inForce].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inForce++;
}
...
void pmpd_tilde_outPos(t_pmpd_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    ...
    x->nb_outPos++;
    pmpd_tilde_need_outlet(x, idx_outlet);
}
