	...
	while (n--) 
	{
		...
		while (x->loop++ < x->nb_loop)
		{
			...
			// compute forces generated by links (spring / dashpot)
			{
				...
				//logpost(x,2, "L%f, LX%f, LY%f, F%f, FX%f, FY%f", L, LX, LY, F, FX, FY);
			...
			// a mass does not move if M=0 (i.e : invM = 0)
				//logpost(x,2, "I:%ld, X:%f, Y:%f, SpeedX%f, speedY%f, forceX%f, ForceY%f", i, x->mass[i].posX, x->mass[i].posY, x->mass[i].speedX, x->mass[i].speedY, x->mass[i].forceX, x->mass[i].forceY );
		...
		for (i=0; i<x->nb_outlet; i++)	// send vector value to the vector pointer
			//if(i==0) *out[i]++ = tmp;
			//else 
			*out[i]++ = x->outlet[i];
	}
...
{ 
	...
	for (i=0; i<x->nb_mass; i++) logpost(x, 2, "mass %ld, M:%f, posX:%f, posY:%f, FX:%f, FY:%f",i, x->mass[i].invM<=0.?0:1/x->mass[i].invM, x->mass[i].posX,x->mass[i].posY,x->mass[i].forceX,x->mass[i].forceY);
...
//invM speedX posX force
	...
	if (x->nb_mass >= nb_max_mass) {
		pd_error((t_object*)x, "to many mass");
		x->nb_mass--;
	}
}
...
// *mass1, *mass2, K1, D1;
	...
	if (x->nb_link >= nb_max_link) {
		pd_error((t_object*)x,"to many link");
		x->nb_link--;
	}
}
...
// add a NLlink
	if  (argc == 8) 
	{
		...
		if (x->nb_NLlink == nb_max_link) {
			pd_error((t_object*)x,"to many NLlink");
			x->nb_NLlink--;
		}
	}
	else
