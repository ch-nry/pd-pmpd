...
{
	if (x->nb_inPos == x->max_in) {
		pd_error(x, "too many inPos");
		return;
	}
...
// nbr_inlet, *mass1, influence;
{
	if (x->nb_inForce == x->max_in) {
		pd_error(x, "too many inForce");
		return;
	}
...
{
	if (x->nb_outPos == x->max_out) {
		pd_error(x, "too many outPos");
		return;
	}
