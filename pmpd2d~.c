...
///////////////////////////////////////////////////////////////////////////////////
{
    ...
    t_float F, FX, FY, L, LX, LY, deltaL, invL, tmpX, tmpY, speed, invSpeed;
    t_int i, si, loop;
...
void pmpd2d_tilde_bang(t_pmpd2d_tilde *x)
{
    ...
    if(x->test_minmax) { logpost(x, 2, "minX:%f, maxX:%f, minY:%f, maxY:%f", x->minX, x->maxX, x->minY, x->maxY); }
}
...
static inline int validate_inlet_io(t_pmpd2d_tilde *x, int idx)
{
    if (idx < 0 || (x->nb_inlet_lim && idx+1 > x->nb_inlet_lim)) {
        pd_error(x, "pmpd2d~: no inlet at index %i", idx);
        return 0;
    }
    return 1;
}

static inline int validate_outlet_io(t_pmpd2d_tilde *x, int idx)
{
    if (idx < 0 || (x->nb_outlet_lim && idx+1 > x->nb_outlet_lim)) {
        pd_error(x, "pmpd2d~: no outlet at index %i", idx);
        return 0;
    }
    return 1;
}

static void pmpd2d_tilde_need_outlet(t_pmpd2d_tilde *x, int idx)
{
...
void pmpd2d_tilde_outPosX(t_pmpd2d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    ...
    x->nb_outPosX++;
    pmpd2d_tilde_need_outlet(x, idx_outlet);
}
