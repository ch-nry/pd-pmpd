...
void *pmpd_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	x->nb_max_mass = (arg = (int)atom_getfloatarg(2, argc, argv)) > 0 ? arg : NB_MAX_MASS_DEFAULT;
	x->nb_max_link = (arg = (int)atom_getfloatarg(3, argc, argv)) > 0 ? arg : NB_MAX_LINK_DEFAULT;
	x->nb_max_in   = max(x->nb_inlet,  (int)atom_getfloatarg(4, argc, argv));
	x->nb_max_out  = max(x->nb_outlet, (int)atom_getfloatarg(5, argc, argv));
