...
typedef struct _pmpd3d_tilde {
    ...
    t_int multichannel;
    t_int nb_inlet_lim, nb_outlet_lim, nb_outlet_want;
    ...
    for (si=0; si<n; si++)
    {
        for (loop=0; loop<x->nb_loop; loop++)
        {
            for (i=0; i<x->nb_inPosX; i++)
                ...
                x->inPosX[i].mass->posX = x->inPosX[i].influence * x->inPosX[i].vec[si];
            for (i=0; i<x->nb_inPosY; i++)
                x->inPosY[i].mass->posY = x->inPosY[i].influence * x->inPosY[i].vec[si];
            for (i=0; i<x->nb_inPosZ; i++)
                x->inPosZ[i].mass->posZ = x->inPosZ[i].influence * x->inPosZ[i].vec[si];
            for (i=0; i<x->nb_inForceX; i++)
                // get inlet value and make it a force to the specified mass
                x->inForceX[i].mass->forceX += x->inForceX[i].influence * x->inForceX[i].vec[si];
            for (i=0; i<x->nb_inForceY; i++)
                x->inForceY[i].mass->forceY += x->inForceY[i].influence * x->inForceY[i].vec[si];
            for (i=0; i<x->nb_inForceZ; i++)
                x->inForceZ[i].mass->forceZ += x->inForceZ[i].influence * x->inForceZ[i].vec[si];
            ...
            if (!x->test_minmax) 
            { // High-performance loop, but accuracy errors can occur.
		    ...
		    { 
		        for (i=0; i<x->nb_mass; i++)
		        { // compute new masses position
				    if(x->mass[i].invM!=0) { // a mass does not move if M=0 (i.e : invM = 0)
						if ( x->mass[i].D && (sqspeed = x->mass[i].speedX * x->mass[i].speedX + x->mass[i].speedY * x->mass[i].speedY + x->mass[i].speedZ * x->mass[i].speedZ) ) 
						{ // velocity damping
					        L = sqrt(sqspeed);
					        invL = 1 / L;
					        F = -(L - x->mass[i].Doffset) * x->mass[i].D;
...
void pmpd3d_tilde_dsp(t_pmpd3d_tilde *x, t_signal **sp)
{
    ...
    if (x->multichannel) {
        // inlet count follows the connected signal
        if (nchans != x->nb_inlet) {
            x->inlet_vector = (t_sample **)resizebytes(x->inlet_vector,
                x->nb_inlet * sizeof(t_sample *), nchans * sizeof(t_sample *));
            x->nb_inlet = nchans;
        }
        // apply pending outlet width from out*
        if (x->nb_outlet_want != x->nb_outlet) {
            x->outlet = (t_float *)resizebytes(x->outlet,
                x->nb_outlet * sizeof(t_float), x->nb_outlet_want * sizeof(t_float));
            x->outlet_vector = (t_sample **)resizebytes(x->outlet_vector,
                x->nb_outlet * sizeof(t_sample *), x->nb_outlet_want * sizeof(t_sample *));
            x->nb_outlet = x->nb_outlet_want;
        }
        for (i=0; i<nchans; i++)
            x->inlet_vector[i] = sp[0]->s_vec + nsamples * i;
    ...
    // wrap in* indexes onto connected channels
    for (i = 0; i < x->nb_inPosX; i++)
        x->inPosX[i].vec = x->inlet_vector[x->inPosX[i].nbr_inlet % x->nb_inlet];
    for (i = 0; i < x->nb_inPosY; i++)
        x->inPosY[i].vec = x->inlet_vector[x->inPosY[i].nbr_inlet % x->nb_inlet];
    for (i = 0; i < x->nb_inPosZ; i++)
        x->inPosZ[i].vec = x->inlet_vector[x->inPosZ[i].nbr_inlet % x->nb_inlet];
    for (i = 0; i < x->nb_inForceX; i++)
        x->inForceX[i].vec = x->inlet_vector[x->inForceX[i].nbr_inlet % x->nb_inlet];
    for (i = 0; i < x->nb_inForceY; i++)
        x->inForceY[i].vec = x->inlet_vector[x->inForceY[i].nbr_inlet % x->nb_inlet];
    for (i = 0; i < x->nb_inForceZ; i++)
        x->inForceZ[i].vec = x->inlet_vector[x->inForceZ[i].nbr_inlet % x->nb_inlet];
...
void pmpd3d_tilde_bang(t_pmpd3d_tilde *x)
{
    ...
    if(x->test_minmax) { logpost(x, 2, "minX:%f, maxX:%f, minY:%f, maxY:%f, minZ:%f, maxZ:%f", x->minX, x->maxX, x->minY, x->maxY, x->minZ, x->maxZ); }
}
...
static inline int validate_inlet_io(t_pmpd3d_tilde *x, int idx)
{
    if (idx < 0 || (x->nb_inlet_lim && idx+1 > x->nb_inlet_lim)) {
        pd_error(x, "pmpd3d~: no inlet at index %i", idx);
        return 0;
    }
    return 1;
}

static inline int validate_outlet_io(t_pmpd3d_tilde *x, int idx)
{
    if (idx < 0 || (x->nb_outlet_lim && idx+1 > x->nb_outlet_lim)) {
        pd_error(x, "pmpd3d~: no outlet at index %i", idx);
        return 0;
    }
    return 1;
}

static void pmpd3d_tilde_need_outlet(t_pmpd3d_tilde *x, int idx)
{
    if (x->nb_outlet_lim) return;
    if (idx+1 > x->nb_outlet_want) {
        x->nb_outlet_want = idx+1;
        canvas_update_dsp();
    }
}
...
void pmpd3d_tilde_inPosX(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inPosX[x->nb_inPosX].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inPosX++;
}
...
void pmpd3d_tilde_inPosY(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inPosY[x->nb_inPosY].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inPosY++;
}
...
void pmpd3d_tilde_inPosZ(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inPosZ[x->nb_inPosZ].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inPosZ++;
}
...
void pmpd3d_tilde_inForceX(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inForceX[x->nb_inForceX].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inForceX++;
}
...
void pmpd3d_tilde_inForceY(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inForceY[x->nb_inForceY].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inForceY++;
}
...
void pmpd3d_tilde_inForceZ(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inForceZ[x->nb_inForceZ].vec = x->inlet_vector[(int)idx_inlet % x->nb_inlet];
    x->nb_inForceZ++;
}
...
void pmpd3d_tilde_outPosX(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_count(x, x->nb_outPosX, x->nb_max_out, "outPosX outlet assignments") ||
        !validate_outlet_io(x, idx_outlet) ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    ...
    x->nb_outPosX++;
    pmpd3d_tilde_need_outlet(x, idx_outlet);
}
...
void pmpd3d_tilde_reset(t_pmpd3d_tilde *x)
{
    ...
    x->nb_outPosZ   = 0;
    x->test_minmax = 0;
    if (!x->nb_outlet_lim && x->nb_outlet_want > 1) {
        x->nb_outlet_want = 1;
        canvas_update_dsp();
    }
}
...
void *pmpd3d_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    x->nb_inlet_lim = max(x->multichannel ? 0 : 1, atom_getintarg(0, argc, argv));
    x->nb_outlet_lim = max(x->multichannel ? 0 : 1, atom_getintarg(1, argc, argv));
    ...
    x->nb_inlet = max(1, x->nb_inlet_lim);
    x->nb_outlet = max(1, x->nb_outlet_lim);
    x->nb_outlet_want = x->nb_outlet;
